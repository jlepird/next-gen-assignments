...
if ($email == "ERROR-- no rows returned") { 
	...
	$_SESSION['isOwner'] = $sql->queryValue("select owner from users where username = '" . $uname . "' and password = '" . $password . "';");
	$_SESSION['isAirman'] = $sql->queryValue("select officer from users where username = '" . $uname . "' and password = '" . $password . "';");
